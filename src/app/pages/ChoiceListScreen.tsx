        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
          ...
          {error && (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 py-20">
              ...
              <button
                onClick={() => { setError(false); setLoading(true); quizApi.getChoiceList().then(setItems).finally(() => setLoading(false)); }}
          ...
          {!loading && !error && items.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 py-20">
              ...
              <p className="text-text-sub text-sm">아직 등록된 퀴즈가 없어요</p>
          ...
          {items.map((item, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => navigate(`/choices/${item.id}`)}
            ...
            >
              <div
                ...
                style={{ background: '#EFF6FF', fontSize: 24 }}
              >
                🃏
              </div>
              <div className="flex-1">
                <p className="text-[15px] font-bold text-text-main">퀴즈 #{item.id}</p>
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-[12px] text-text-sub">🪙 {item.solvedCoin.toLocaleString()} 코인</span>
                  <span className="text-[12px] text-text-sub">플레이 {item.cnt}회</span>
                </div>
              </div>
